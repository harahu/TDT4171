...
def importanceEntr(attribute, examples):
	...
	for e in examples:
		if ( e[attribute] == 1 ):
			...
			l2.append(e)
	remainder = (len(l1)/len(examples))*setEntr(l1) + (len(l2)/len(examples))*setEntr(l2)
	gain = goal - remainder
	return(gain)
...
def main():
	...
	for e in testdata:
		...
		if ( e[7] != treeEnt.classifyExample(e) ):
			ec += 1
		print(str(e[7])+str(treeRnd.classifyExample(e))+"\t"+str(e[7])+str(treeEnt.classifyExample(e)))
	print("\n")
	print("Total Error:")
	print(str(rc)+"\t"+str(ec))
	print("of\tof")
